...
void ai_updateGameData(fix32 ball_pos_x, fix32 ball_pos_z)
{
	...
	ai.target_pos_z = RSE_fix32Mul(board_length, FIX32(-0.45));
	...
	ai.racket_speed = fix32RangeAdjust(ai.racket_speed, FIX32(0.0), FIX32(1.0), ai.max_racket_speed, RSE_fix32Mul(ai.max_racket_speed, FIX32(0.01)));
}
...
void ai_update(fix32 dt)
{
	...
	ai.pos_x = fix32Add(ai.pos_x, RSE_fix32Mul(fix32Sub(ai.target_pos_x, ai.pos_x), RSE_fix32Mul(dt, ai.racket_speed)));
	ai.pos_z = fix32Add(ai.pos_z, RSE_fix32Mul(fix32Sub(ai.target_pos_z, ai.pos_z), RSE_fix32Mul(dt, ai.racket_speed)));
